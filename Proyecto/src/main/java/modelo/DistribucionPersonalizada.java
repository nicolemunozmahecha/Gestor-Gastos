...
public class DistribucionPersonalizada implements EstrategiaDistribucion {
    private Map<Persona, Double> porcentajes;
    
    public DistribucionPersonalizada(Map<Persona, Double> porcentajes) {

        double suma = porcentajes.values().stream().mapToDouble(Double::doubleValue).sum();
        this.porcentajes = new HashMap<>(porcentajes);
    }

    public static DistribucionPersonalizada crearConPorcentajes(Map<Persona, Double> porcentajesPorCien) {
        ...
        return new DistribucionPersonalizada(porcentajesNormalizados);
    }
